...
- `echo [args...]`:
  - печатает args, разделяя пробелом, без переноса строки.
- `pwd`:
...
> Ключевой момент из чеклиста: **аргументы и входной поток — разные вещи**. Например, `echo 123 | wc` передаёт “123\n” через stdin, а не как argv.
